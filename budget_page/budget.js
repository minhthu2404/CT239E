...
async function loadBudgetData() {
    ...
    try {
        // Fetch budgets
        const budgetsRes = await fetch(`http://localhost:3000/api/budgets?username=${user.username}&month=${monthStr}`);
        if (budgetsRes.ok) {
            currentBudgets = await budgetsRes.json();
        }

        // Fetch all transactions and filter for current month
        const transRes = await fetch(`http://localhost:3000/api/transactions?username=${user.username}`);
        if (transRes.ok) {
...
function updateUI() {
    ...
    budgetElements.forEach(el => {
        ...
        if (budgetObj) {
            ...
            const dailyAvg = remaining > 0 ? Math.floor(remaining / remainingDays) : 0;

            const percent = budgetObj.amount > 0 ? Math.min(Math.round((spent / budgetObj.amount) * 100), 100) : 0;
...
function openForm(categoryName, iconClass) {
    ...
    const budgetObj = currentBudgets.find(b => b.category === categoryMap[categoryName]);
